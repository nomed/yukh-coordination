...
// NewRootKeySource is the injectible native RFC-0027 Keychain root-key constructor.
...
func (f *Factory) Open(ctx context.Context, configPath string, tokenFD int) (*BootstrapProfile, error) {
	...
	issuer, err := clientauth.NewHTTPSessionIssuer(config.RelayBaseURI(), f.dependencies.Transport)
	if err != nil {
		_ = token.Close()
		_ = store.Close()
		return nil, clientauth.ErrInvalidCredential
