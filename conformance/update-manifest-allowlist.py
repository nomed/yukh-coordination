...
files={*(p for p in out.rglob("*") if p.is_file() and p.name not in {"SHA256SUMS","manifest-inputs.txt"} and "__pycache__" not in p.parts and p.suffix != ".pyc"), *ROOT.joinpath("schema").rglob("*.json"), *ROOT.joinpath("js").rglob("*.mjs"), *ROOT.joinpath("test").rglob("*.mjs"), ROOT/"package.json", ROOT/".gitignore", ROOT/"docs/rfc/0001-protocol-v0.1.md", ROOT/"docs/rfc/0002-mvp-security-boundary.md", target}
